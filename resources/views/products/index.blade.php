...
@section('content')
<div class="page-hero">
    <div class="breadcrumb"><a href="{{ route('home') }}">Home</a> / Products</div>
...
<div class="shop-layout">
    ...
    <aside class="sidebar">
        <form method="GET" action="{{ route('products.index') }}">
            <div class="sidebar-card">
                ...
                @php $categories = ['Electronics', 'Fashion', 'Home & Living', 'Beauty', 'Sports', 'Books']; @endphp
                @foreach($categories as $cat)
                <label class="filter-label">
                    <input type="checkbox" name="category[]" value="{{ strtolower($cat) }}"
                        {{ in_array(strtolower($cat), request('category', [])) ? 'checked' : '' }}>
                    {{ $cat }}
                </label>
                @endforeach
            </div>
            <div class="sidebar-card">
                ...
                <div class="price-inputs">
                    <input type="number" class="price-input" name="min_price" placeholder="Min" value="{{ request('min_price') }}">
                    <input type="number" class="price-input" name="max_price" placeholder="Max" value="{{ request('max_price') }}">
                </div>
            </div>
            <button type="submit" class="btn-filter"><i class="fas fa-filter"></i> Apply Filters</button>
            <a href="{{ route('products.index') }}" style="display:block;text-align:center;margin-top:0.8rem;color:var(--text-muted);font-size:0.85rem;text-decoration:none;">Clear All</a>
        </form>
    </aside>
    ...
    <div>
        <div class="products-header">
            <span class="products-count">Showing {{ $products->count() ?? 0 }} products</span>
        ...
        <div class="products-grid">
            ...
            <div class="product-card">
                <div class="product-img">
                    ...
                    <div class="product-actions">
                        ...
                        <a href="{{ route('products.show', $product->id) }}" class="btn-view"><i class="fas fa-eye"></i> View</a>
                    </div>
                </div>
                <div class="product-info">
                    ...
                    <div class="product-footer">
                        ...
                        <div class="product-stock {{ ($product->stock ?? 10) < 5 ? 'low' : '' }}">
                            {{ ($product->stock ?? 10) < 5 ? 'Low Stock' : 'In Stock' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="product-card" style="cursor:pointer">
                ...
                <div class="product-info">
                    ...
                    <div class="product-name">Luxury Product No. {{ $i }}</div>
                    <div class="product-footer">
                        <div class="product-price">PKR {{ number_format(rand(1500,25000), 0) }}</div>
